...
class Utility {
	...
	public function register_notice() {
		...
		if ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$message = sprintf(
				...
				__( 'Your license key doesn\'t match your current domain. Please activate the license again for your current domain.', 'shopbuilderwp' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'shopbuilderwp' ),
			);
		} else {
			$message = sprintf(
				'<b>%1$s <a href="%2$s" target="_blank">%3$s</a> %4$s <a href="%5$s">%6$s</a></b>',
				__( 'Please activate your theme using Envato', 'shopbuilderwp' ),
				esc_url( 'https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-' ),
				__( 'purchase code', 'shopbuilderwp' ),
				__( 'to get full theme functionality and customer support.', 'shopbuilderwp' ),
				menu_page_url( 'rtlc', false ),
				__( 'Activate License', 'shopbuilderwp' ),
			);
		}
	...
	public function script() {
					...
					if (purchase_code) {
						jQuery.ajax({
							...
							success: function (resp) {
								if (resp === 555) {
									alert('<?php esc_html_e( 'Purchase code already activated for one domain!!!', 'shopbuilderwp' ); ?>');
								} else {
									if (resp) {
										jQuery('.rtlc-status-btn').html('<?php esc_html_e( 'Activated', 'shopbuilderwp' ); ?>');
										jQuery('.rtlc-status-btn').removeClass('rtlc-unverified').addClass('rtlc-verified');
									} else {
										alert('<?php esc_html_e( 'Sorry!!! Purchase code does not match', 'shopbuilderwp' ); ?>');
									}
								}
						...
						alert('<?php esc_html_e( 'Purchase code is required!!!', 'shopbuilderwp' ); ?>');
					}
				});
			</script>
			<?php
		...
		if ( wp_script_is( 'jquery', 'done' ) && ! rtlc_is_valid()['success'] ) {
			if ( isset( $_GET['page'] ) && 'fw-backups-demo-content' === $_GET['page'] ) {
				...
				<script type="text/javascript">
					jQuery("#fw-ext-backups-demo-list .theme-actions a").on("click", function (e) {
						e.preventDefault();
						alert('<?php esc_html_e( 'Please activate your theme using Envato purchase code, to install demo data.', 'shopbuilderwp' ); ?>');
						return false;
					});
				</script>
				<?php
			...
			if ( isset( $_GET['page'] ) && 'finwave-install-plugins' === $_GET['page'] ) {
				?>
				<script type="text/javascript">
					jQuery(".row-actions a").on("click", function (e) {
						if (jQuery(this).closest('td').next().has('span').length > 0) { //find pre packaged
							e.preventDefault();
							alert('<?php esc_html_e( 'Please activate your theme using Envato purchase code, to use this plugin.', 'shopbuilderwp' ); ?>');
							return false;
						}
					});
...
if ( ! function_exists( 'rtlc_is_valid' ) ) {
	...
	function rtlc_is_valid() {
		...
		$finwave      = $theme_info->get( 'Name' );

		$finwave      = strtolower( trim( preg_replace( '/[^A-Za-z0-9-]+/', '-', $finwave ) ) );
		$get_option = get_option( 'rt_licenses' );

		if ( isset( $get_option[ $finwave . '_license_key' ] ) ) {
			return [
				'success'      => true,
				'domain_match' => true,
			];
		} elseif ( isset( $get_option[ $finwave . '_license' ] ) && isset( $get_option[ $finwave . '_license' ]['key'] ) ) {
			$domain_name = rtlc_get_domain_name();
			$domain      = $get_option[ $finwave . '_license' ]['domain'];
