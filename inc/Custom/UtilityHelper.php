...
class UtilityHelper {
	...
	public function theme_menu() {
		add_theme_page( esc_html__( 'Theme License', 'shopbuilderwp' ), esc_html__( 'Theme License', 'shopbuilderwp' ), 'manage_options', 'rtlc', [ $this, 'create_admin_page' ], null, 99 );
	}
	...
	public function create_admin_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Theme License', 'shopbuilderwp' ); ?></h1>
			<form method="post" action="options.php">
				<?php
				// This prints out all hidden setting fields.
				settings_fields( 'rt_option_group' );
				do_settings_sections( 'finwave-setting' );
				?>
			</form>
		</div>
		<?php
	...
	public function theme_option() {
		...
		add_settings_field(
			'rt_purchase_code',
			esc_html__( 'Purchase Code', 'shopbuilderwp' ),
		...
		add_settings_field(
			'rt_license_status',
			esc_html__( 'License Status', 'shopbuilderwp' ),
	...
	public function license_status_callback() {
		...
		$status_text = esc_html__( 'Not Activated', 'shopbuilderwp' );

		if ( rtlc_is_valid()['success'] ) {
			$verify      = true;
			$status_text = __( 'Activated', 'shopbuilderwp' );
		} elseif ( isset( rtlc_is_valid()['domain_match'] ) && ! rtlc_is_valid()['domain_match'] ) {
			$status_text = __( 'Domain Mismatch', 'shopbuilderwp' );
		}
	...
	public function license_note_callback() {
		...
		/* translators: Support Center */
			__( 'Please keep in mind, you can activate one license in one domain, if you face any problem in activation, please contact our <a href="%s" target="_blank">Support Center</a>', 'shopbuilderwp' ),
			esc_url( $support )
		);
	...
	public function license_check_callback() {
		printf(
			...
			esc_html__( 'Activate License', 'shopbuilderwp' )
		);
	}
	...
	public function rtlc_verification() {
		...
		if ( $purchase_code ) {
			...
			$finwave = $this->finwave;
			$domain_name     = rtlc_get_domain_name();

			$api_url     = "{$rt_license_server}/?theme_name={$finwave}&purchase_code=" . $purchase_code . '&domain_name=' . $domain_name;
