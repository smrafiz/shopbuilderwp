...
if ( ! function_exists( 'shopbuilderwp_menu_icons_group' ) ) {
	/**
	 * Get menu icon group
	 * @return void
	 */
	function shopbuilderwp_menu_icons_group( $args = [] ) {
		$default_args = [
			'hamburg'       => shopbuilderwp_option( 'rt_header_bar' ),
			'search'        => shopbuilderwp_option( 'rt_header_search' ),
			'login'         => shopbuilderwp_option( 'rt_header_login_link' ),
			'button'        => shopbuilderwp_option( 'rt_get_started_button' ),
			'button_label'  => shopbuilderwp_option( 'rt_get_started_label' ),
			'button_link'   => shopbuilderwp_option( 'rt_get_started_button_url' ),
			'has_separator' => shopbuilderwp_option( 'rt_header_separator' ),
			'compare'    	=> shopbuilderwp_option( 'rt_header_compare' ),
			'wishlist'    	=> shopbuilderwp_option( 'rt_header_wishlist' ),
			'add_to_cart' 	=> shopbuilderwp_option( 'rt_header_add_to_cart' )
		];
		...
		<div class="menu-icon-wrapper ml-auto">
			<ul class="menu-icon-action <?php echo esc_attr( $menu_classes ) ?>">
				<?php if ( $args['hamburg'] ) : ?>
					<?php shopbuilderwp_hanburger( 'desktop-hamburg' ); ?>
				<?php endif; ?>

				<?php shopbuilderwp_hanburger( 'mobile-hamburg' ); ?>
				...
				<?php if ( $args['login'] ) : ?>
					<li class="rt-user-login rt-button">
						<a  class="btn button-4" href="<?php echo esc_url( wp_login_url() ) ?>" aria-label="user login">
							<?php if ( shopbuilderwp_option( 'rt_get_login_label' ) ) { ?><?php echo shopbuilderwp_option( 'rt_get_login_label' ) ?><?php } ?>
						</a>
					</li>
				<?php endif; ?>
...
if ( ! function_exists( 'shopbuilderwp_require' ) ) {
	/**
	 ...
	 * @return false|void
	 */
	function shopbuilderwp_require( $filename, string $dir = 'inc' ) {
...
if ( ! function_exists( 'shopbuilderwp_readmore_text' ) ) {
	...
	function shopbuilderwp_readmore_text() {
		...
		if ( empty( shopbuilderwp_option( 'rt_blog_read_more' ) ) ) {
			return;
		}
		return sprintf(
			'%s %s',
			esc_html( shopbuilderwp_option( 'rt_blog_read_more' ) ),
...
if ( ! function_exists( 'shopbuilderwp_get_svg' ) ) {
	...
	function shopbuilderwp_get_svg( $name, $rotate = '' ) {
		$svg_list     = apply_filters( 'shopbuilderwp_svg_icon_list', [
...
if ( ! function_exists( 'shopbuilderwp_option' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function shopbuilderwp_option( $key, $default = '', $return_array = false ) {
...
if ( ! function_exists( 'shopbuilderwp_site_logo' ) ) {
	 ...
	 *
	 */
	function shopbuilderwp_site_logo( $with_logo = false, $custom_title = '', $mode = '' ) {
		$logo_main       = shopbuilderwp_option( 'rt_logo' );
		$logo_light      = shopbuilderwp_option( 'rt_logo_light' );
		$logo_mobile     = shopbuilderwp_option( 'rt_logo_mobile' );
...
if ( ! function_exists( 'shopbuilderwp_footer_logo' ) ) {
	/**
	 * Newfit Site Logo
	 *
	 */
	function shopbuilderwp_footer_logo() {
		$main_logo  = shopbuilderwp_option( 'rt_logo' );
		$logo_light = shopbuilderwp_option( 'rt_logo_light' );
		$site_logo  = $main_logo;
...
if ( ! function_exists( 'shopbuilderwp_scroll_top' ) ) {
	...
	function shopbuilderwp_scroll_top( $class = '', $icon = 'scroll-top' ) {
		if ( shopbuilderwp_option( 'rt_back_to_top' ) ) {
			?>
...
if ( ! function_exists( 'shopbuilderwp_post_thumbnail' ) ) {
	 ...
	 * @return void
	 */
	function shopbuilderwp_post_thumbnail( $size = 'full', $thumb_date = false ) {
...
if ( ! function_exists( 'shopbuilderwp_post_single_thumbnail' ) ) {
	...
	function shopbuilderwp_post_single_thumbnail( $size = 'full' ) {
		...
		<?php } else { ?>
			...
			<?php if ( wp_get_attachment_caption( get_post_thumbnail_id() ) && shopbuilderwp_option( 'rt_single_caption_visibility' ) == 1 ) : ?>
				<figcaption class="wp-caption-text">
					<span><?php shopbuilderwp_html( wp_get_attachment_caption( get_post_thumbnail_id() ) ); ?></span>
				</figcaption>
			<?php endif; ?>
		<?php } ?>
		</div>
...
if ( ! function_exists( 'shopbuilderwp_entry_footer' ) ) {
	...
	function shopbuilderwp_entry_footer() {
		if ( ! is_single() ) {
			if ( shopbuilderwp_option( 'rt_blog_footer_visibility' ) ) { ?>
				<footer class="entry-footer rt-button">
					<a class="btn <?php echo esc_attr( shopbuilderwp_option('rt_blog_btn_style') ); ?>"
					   href="<?php echo esc_url( get_permalink() ) ?>" <?php if( is_numeric(shopbuilderwp_option('rt_blog_btn_radius') ) ) { ?>
						style="border-radius: <?php echo esc_attr( shopbuilderwp_option('rt_blog_btn_radius') ); ?>px"<?php } ?>><?php echo shopbuilderwp_readmore_text() ?><i class="icon-rt-right-arrow"></i>
					</a>
				</footer>
			<?php }
		} else {
			if ( ( has_tag() && shopbuilderwp_option( 'rt_single_tag_visibility' ) ) || shopbuilderwp_option( 'rt_single_share_visibility' ) ) { ?>
				<footer class="entry-footer d-flex align-items-center justify-content-between">
					<?php if ( shopbuilderwp_option( 'rt_single_tag_visibility' ) ) { ?>
						<div class="post-tags">
							<?php if ( $tags_label = shopbuilderwp_option( 'rt_tags' ) ) {
					...
					<?php }
					if ( shopbuilderwp_option( 'rt_single_share_visibility' ) ) { ?>
						<div class="post-share">
							<?php if ( $tags_label = shopbuilderwp_option( 'rt_social' ) ) {
								printf( "<span>%s</span>", esc_html( $tags_label ) );
							} ?>
							<?php PostShare::shopbuilderwp_post_share(); ?>
						</div>
					<?php } ?>
				</footer>
				<?php
...
// single video
if ( ! function_exists( 'shopbuilderwp_post_single_video' ) ) {
	/**
	 * Display post details video
	 * @return void
	 */
	function shopbuilderwp_post_single_video() { ?>
		<?php $rt_youtube_link = get_post_meta( get_the_ID(), 'rt_youtube_link', true );
...
if ( ! function_exists( 'shopbuilderwp_entry_profile' ) ) {
	/**
	 * Finwave Entry Profile
	 *
	 * @return void
	 *
	 */
	function shopbuilderwp_entry_profile() {
		if ( shopbuilderwp_option( 'rt_single_profile_visibility' ) ) {
					...
					<div class="profile-content">
						<div class="profile-author-info">
							...
							<div class="profile-info">
								...
								<?php if ( $prof_phone ) { ?>
									<span class="profile-phone"><?php echo esc_html__( 'Ph: ', 'shopbuilderwp' ) ?><?php echo esc_html( $prof_phone );?></span>
								<?php } ?>
							</div>
						</div>
...
if ( ! function_exists( 'shopbuilderwp_single_post_footer_meta' ) ) {
	...
	function shopbuilderwp_single_post_footer_meta( $class = '', $includes = [ 'tag' ] ) {
		if ( is_single() && shopbuilderwp_option( 'rt_single_tag_visibility' ) ) : ?>
			<div class="post-footer-meta <?php echo esc_attr( $class ) ?>">
				<?php echo shopbuilderwp_post_meta( [
					'with_list' => false,
					'with_icon' => false,
					'include'   => $includes,
				] ); ?>
...
if ( ! function_exists( 'shopbuilderwp_entry_content' ) ) {
	...
	function shopbuilderwp_entry_content() {
		if ( ! is_single() ) {
			$length = shopbuilderwp_option( 'rt_excerpt_limit' );
...
if ( ! function_exists( 'shopbuilderwp_sidebar' ) ) {
	...
	function shopbuilderwp_sidebar( $sidebar_id ) {
		...
		?>
		<aside id="sidebar" class="finwave-widget-area sidebar-sticky <?php echo esc_attr( $sidebar_cols ) ?>"
			   role="complementary">
...
if ( ! function_exists( 'shopbuilderwp_post_class' ) ) {
	 ...
	 * @return string
	 */
	function shopbuilderwp_post_class( $default_class = 'finwave-post-card rt-grid-item' ) {
		$above_meta_style = 'above-' . shopbuilderwp_option( 'rt_single_above_meta_style' );


		$common_clsss = '';
		if ( is_single() ) {
			$common_clsss .= shopbuilderwp_option( 'rt_single_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = shopbuilderwp_option( 'rt_single_meta_style' );
			$post_classes = Fns::class_list( [ $common_clsss, $meta_style, $above_meta_style ] );
		} else {
			$common_clsss .= shopbuilderwp_option( 'rt_blog_above_meta_visibility' ) ? 'is-above-meta' : 'no-above-meta';
			$meta_style   = shopbuilderwp_option( 'rt_blog_meta_style' );
			$blog_style   = 'blog-' . shopbuilderwp_option( 'rt_blog_style' );
...
if ( ! function_exists( 'shopbuilderwp_separate_meta' ) ) {
	...
	function shopbuilderwp_separate_meta( $class = '', $includes = [ 'category' ] ) {
		if ( ( ! is_single() && shopbuilderwp_option( 'rt_blog_above_meta_visibility' ) ) || ( is_single() && shopbuilderwp_option( 'rt_single_above_meta_visibility' ) ) ) : ?>
...
if ( ! function_exists( 'shopbuilderwp_single_entry_header' ) ) {
	...
	function shopbuilderwp_single_entry_header() {
		...
		<header class="entry-header">
			...
			if ( ! empty( Fns::single_meta_lists() ) && shopbuilderwp_option( 'rt_single_meta_visibility' ) ) {
...
if ( ! function_exists( 'shopbuilderwp_get_avatar_url' ) ) :
	function shopbuilderwp_get_avatar_url( $get_avatar ) {
...
if ( ! function_exists( 'shopbuilderwp_hanburger' ) ) {
	 ...
	 * @return void
	 */
	function shopbuilderwp_hanburger( $class = '' ) {
		?>
		<li class="ham-burger <?php echo esc_attr( $class ) ?>">
			<button type="button" class="menu-bar trigger-off-canvas" aria-label="hamburger menu">
				<?php if( shopbuilderwp_option('rt_get_menu_label') ) { ?>
					<span class="menu-label"><?php echo esc_html( shopbuilderwp_option('rt_get_menu_label') ) ?></span>
				<?php } ?>
